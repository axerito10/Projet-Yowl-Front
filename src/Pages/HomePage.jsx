    ...
    const fetchUserData = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (data && data.data && data.data.attributes && data.data.attributes.groupeFollow.data) {
                    const banners = await Promise.all(data.data.attributes.groupeFollow.data.map(async (groupe) => {
                        try {
...
<section className="container mx-auto px-4 my-8">
      ...
      <section className="container mx-auto px-4 my-8">
        <h2 className="text-2xl font-bold mb-4 text-custom-blue">VOS GROUPES</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {groupFollow.map((follow) => (
            <Link to={`/group/${follow.groupID}`} key={follow.groupID}>
              <div className="group card relative p-5 transition-transform duration-300 hover:scale-105">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out">
                  {follow ? (
                    <>
                      <h3 className="text-lg text-center font-bold mb-2">{follow.Titre}</h3>
                    </>
                  ) : (
